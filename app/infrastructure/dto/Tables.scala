...
trait Tables {
   ...
   *  @param detail Database column detail SqlType(text), Default(None)
   *  @param userId Database column user_id SqlType(int8) */
  case class TasksRow(id: Long, name: String, detail: Option[String] = None, userId: Long)
  /** GetResult implicit for fetching TasksRow objects using plain SQL queries */
  implicit def GetResultTasksRow(implicit e0: GR[Long], e1: GR[String], e2: GR[Option[String]]): GR[TasksRow] = GR{
    prs => import prs._
  ...
  class Tasks(_tableTag: Tag) extends profile.api.Table[TasksRow](_tableTag, "tasks") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(name), detail, Rep.Some(userId))).shaped.<>({r=>import r._; _1.map(_=> TasksRow.tupled((_1.get, _2.get, _3, _4.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))

    /** Database column id SqlType(bigserial), AutoInc, PrimaryKey */
    val id: Rep[Long] = column[Long]("id", O.AutoInc, O.PrimaryKey)
    /** Database column name SqlType(text) */
    val name: Rep[String] = column[String]("name")
    /** Database column detail SqlType(text), Default(None) */
    val detail: Rep[Option[String]] = column[Option[String]]("detail", O.Default(None))
