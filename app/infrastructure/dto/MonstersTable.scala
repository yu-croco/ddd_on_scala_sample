...
trait MonstersTable {
  ...
  case class MonstersRow(id: Long, name: String, life: Long, defensePower: Long, attackPower: Long)
  ...
  implicit def GetResultMonstersRow(implicit e0: GR[Long], e1: GR[String]): GR[MonstersRow] = GR{
    prs => import prs._
    MonstersRow.tupled((<<[Long], <<[String], <<[Long], <<[Long], <<[Long]))
  }
  ...
  class Monsters(_tableTag: Tag) extends profile.api.Table[MonstersRow](_tableTag, "monsters") {
    def * = (id, name, life, defensePower, attackPower) <> (MonstersRow.tupled, MonstersRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(name), Rep.Some(life), Rep.Some(defensePower), Rep.Some(attackPower))).shaped.<>({r=>import r._; _1.map(_=> MonstersRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
